          ...
          {projects.map((project) => (
            <Card key={project.id} className="h-full transition-shadow hover:shadow-sm">
              <CardHeader className="border-b">
                ...
                <RichText value={project.abstract} fallback="No project abstract has been added yet." className="line-clamp-3 min-h-[3.75rem] text-xs/relaxed text-muted-foreground" />
              </CardHeader>
              <CardContent className="flex flex-1 flex-col gap-4 pt-1">
                <div><div className="mb-1.5 flex justify-between text-[10px]"><span className="text-muted-foreground">Research progress</span><span>{project.progress}%</span></div><progress value={project.progress} max={100} aria-label={`${project.title} progress`} className="h-1.5 w-full overflow-hidden rounded-full bg-muted accent-primary [&::-moz-progress-bar]:bg-primary [&::-webkit-progress-bar]:bg-muted [&::-webkit-progress-value]:bg-primary" /></div>
